...
CREATE SEQUENCE AM_GATEWAY_TIER_MAPPING_PK_SEQ START WITH 1 INCREMENT BY 1 NOCACHE
/
CREATE TABLE AM_GATEWAY_TIER_MAPPING (
    ID INTEGER NOT NULL,
    GATEWAY_UUID VARCHAR(45) NOT NULL,
    LOCAL_TIER_NAME VARCHAR(255) NOT NULL,
    REMOTE_PLAN_REFERENCE CLOB NOT NULL,
    FOREIGN KEY (GATEWAY_UUID) REFERENCES AM_GATEWAY_ENVIRONMENT(UUID) ON DELETE CASCADE,
    PRIMARY KEY (ID),
    UNIQUE (GATEWAY_UUID, LOCAL_TIER_NAME))
/
CREATE OR REPLACE TRIGGER AM_GATEWAY_TIER_MAPPING_TRIGGER
BEFORE INSERT
ON AM_GATEWAY_TIER_MAPPING
REFERENCING NEW AS NEW
FOR EACH ROW
    BEGIN
        SELECT AM_GATEWAY_TIER_MAPPING_PK_SEQ.nextval INTO :NEW.ID FROM dual;
    END;
/
...
CREATE TABLE AM_SUBSCRIPTION_EXTERNAL_MAPPING (
    ...
    SUBSCRIPTION_UUID VARCHAR2(256) NOT NULL,
    GATEWAY_ENV_ID VARCHAR2(255) NOT NULL,
...
CREATE TABLE AM_EXTERNAL_CREDENTIAL (
    UUID VARCHAR2(255) NOT NULL,
    MAPPING_UUID VARCHAR2(255),
    APPLICATION_ID VARCHAR2(255) NOT NULL,
    API_ID VARCHAR2(255) NOT NULL,
    AUTHZ_USER VARCHAR2(255) NOT NULL,
    GATEWAY_ENV_ID VARCHAR2(255) NOT NULL,
    NAME VARCHAR2(255),
    EXTERNAL_SUBSCRIPTION_ID VARCHAR2(512),
    REFERENCE_ARTIFACT BLOB,
    CREATED_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    LAST_UPDATED_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE AM_API_FEDERATION_CONFIG (
    API_UUID VARCHAR2(255) NOT NULL,
    GATEWAY_ENV_ID VARCHAR2(255) NOT NULL,
    PUBLISHER_CURATED_CONFIG BLOB,
    GATEWAY_SNAPSHOT_HASH VARCHAR2(64),
    CREATED_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    LAST_UPDATED_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    PUBLISHER_REVIEWED_TIME TIMESTAMP,
    FOREIGN KEY (API_UUID) REFERENCES AM_API(API_UUID) ON DELETE CASCADE,
    FOREIGN KEY (GATEWAY_ENV_ID) REFERENCES AM_GATEWAY_ENVIRONMENT(UUID),
