...
-- Gateway Tier Mapping Table --
IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[AM_GATEWAY_TIER_MAPPING]') AND TYPE IN (N'U'))
CREATE TABLE AM_GATEWAY_TIER_MAPPING (
    ID INTEGER NOT NULL IDENTITY,
    GATEWAY_UUID VARCHAR(45) NOT NULL,
    LOCAL_TIER_NAME VARCHAR(255) NOT NULL,
    REMOTE_PLAN_REFERENCE NVARCHAR(MAX) NOT NULL,
    FOREIGN KEY (GATEWAY_UUID) REFERENCES AM_GATEWAY_ENVIRONMENT(UUID) ON DELETE CASCADE,
    PRIMARY KEY (ID),
    UNIQUE (GATEWAY_UUID, LOCAL_TIER_NAME)
);
...
IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[AM_SUBSCRIPTION_EXTERNAL_MAPPING]') AND TYPE IN (N'U'))
CREATE TABLE AM_SUBSCRIPTION_EXTERNAL_MAPPING (
    ...
    FOREIGN KEY (GATEWAY_ENV_ID) REFERENCES AM_GATEWAY_ENVIRONMENT(UUID),
    PRIMARY KEY (UUID),
    UNIQUE (SUBSCRIPTION_UUID, GATEWAY_ENV_ID)
);

IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[AM_EXTERNAL_CREDENTIAL]') AND TYPE IN (N'U'))
CREATE TABLE AM_EXTERNAL_CREDENTIAL (
    UUID VARCHAR(255) NOT NULL,
    MAPPING_UUID VARCHAR(255),
    ...
    CREATED_TIME DATETIME DEFAULT GETDATE(),
    LAST_UPDATED_TIME DATETIME DEFAULT GETDATE(),
    FOREIGN KEY (MAPPING_UUID) REFERENCES AM_SUBSCRIPTION_EXTERNAL_MAPPING(UUID) ON DELETE SET NULL,
    FOREIGN KEY (GATEWAY_ENV_ID) REFERENCES AM_GATEWAY_ENVIRONMENT(UUID),
    PRIMARY KEY (UUID)
);

IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[AM_API_FEDERATION_CONFIG]') AND TYPE IN (N'U'))
CREATE TABLE AM_API_FEDERATION_CONFIG (
    API_UUID VARCHAR(255) NOT NULL,
    GATEWAY_ENV_ID VARCHAR(255) NOT NULL,
    PUBLISHER_CURATED_CONFIG VARBINARY(MAX),
    GATEWAY_SNAPSHOT_HASH VARCHAR(64),
    CREATED_TIME DATETIME DEFAULT GETDATE(),
    LAST_UPDATED_TIME DATETIME DEFAULT GETDATE(),
    PUBLISHER_REVIEWED_TIME DATETIME,
    FOREIGN KEY (API_UUID) REFERENCES AM_API(API_UUID) ON DELETE CASCADE,
    FOREIGN KEY (GATEWAY_ENV_ID) REFERENCES AM_GATEWAY_ENVIRONMENT(UUID),
    PRIMARY KEY (API_UUID, GATEWAY_ENV_ID)
);
