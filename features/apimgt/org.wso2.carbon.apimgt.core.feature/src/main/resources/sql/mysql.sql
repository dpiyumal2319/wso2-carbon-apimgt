...
CREATE TABLE IF NOT EXISTS AM_GATEWAY_TIER_MAPPING (
    ...
    REMOTE_PLAN_REFERENCE JSON NOT NULL,
    FOREIGN KEY (GATEWAY_UUID) REFERENCES AM_GATEWAY_ENVIRONMENT(UUID) ON DELETE CASCADE,
    PRIMARY KEY (ID),
...
CREATE TABLE IF NOT EXISTS AM_SUBSCRIPTION_EXTERNAL_MAPPING (
    ...
    FOREIGN KEY (GATEWAY_ENV_ID) REFERENCES AM_GATEWAY_ENVIRONMENT(UUID),
    PRIMARY KEY (UUID),
    UNIQUE KEY UQ_SUB_ENV (SUBSCRIPTION_UUID, GATEWAY_ENV_ID)
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS AM_API_FEDERATION_CONFIG (
    API_UUID VARCHAR(255) NOT NULL,
    GATEWAY_ENV_ID VARCHAR(255) NOT NULL,
    PUBLISHER_CURATED_CONFIG LONGBLOB,
    GATEWAY_SNAPSHOT_HASH VARCHAR(64),
    CREATED_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    LAST_UPDATED_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PUBLISHER_REVIEWED_TIME TIMESTAMP NULL,
    FOREIGN KEY (API_UUID) REFERENCES AM_API(API_UUID) ON DELETE CASCADE,
    FOREIGN KEY (GATEWAY_ENV_ID) REFERENCES AM_GATEWAY_ENVIRONMENT(UUID),
