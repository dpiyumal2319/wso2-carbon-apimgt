...
public class SQLConstants {
  ...
  //--------------------
    ...
    public static final String INSERT_GATEWAY_TIER_MAPPING_SQL = "INSERT INTO AM_GATEWAY_TIER_MAPPING " +
            "(GATEWAY_UUID, LOCAL_TIER_NAME, REMOTE_PLAN_REFERENCE) VALUES (?, ?, ?)";

    public static final String GET_GATEWAY_TIER_MAPPINGS_SQL = "SELECT LOCAL_TIER_NAME, REMOTE_PLAN_REFERENCE " +
            "FROM AM_GATEWAY_TIER_MAPPING WHERE GATEWAY_UUID = ?";

    public static final String DELETE_GATEWAY_TIER_MAPPINGS_SQL =
            "DELETE FROM AM_GATEWAY_TIER_MAPPING WHERE GATEWAY_UUID = ?";
    ...
    // Subscription External Mapping SQL Statements
    public static final String ADD_SUBSCRIPTION_EXTERNAL_MAPPING_SQL = "INSERT INTO AM_SUBSCRIPTION_EXTERNAL_MAPPING " +
            "(UUID, SUBSCRIPTION_UUID, GATEWAY_ENV_ID, REFERENCE_ARTIFACT) VALUES (?, ?, ?, ?)";

    public static final String GET_SUBSCRIPTION_EXTERNAL_MAPPING_SQL = "SELECT UUID, REFERENCE_ARTIFACT, " +
            "CREATED_TIME, LAST_UPDATED_TIME " +
            "FROM AM_SUBSCRIPTION_EXTERNAL_MAPPING WHERE SUBSCRIPTION_UUID = ? AND GATEWAY_ENV_ID = ?";

    public static final String GET_SUBSCRIPTION_EXTERNAL_MAPPING_BY_UUID_SQL = "SELECT UUID, SUBSCRIPTION_UUID, " +
            "GATEWAY_ENV_ID, REFERENCE_ARTIFACT, CREATED_TIME, LAST_UPDATED_TIME " +
            "FROM AM_SUBSCRIPTION_EXTERNAL_MAPPING WHERE UUID = ?";

    public static final String UPDATE_SUBSCRIPTION_EXTERNAL_MAPPING_SQL = "UPDATE AM_SUBSCRIPTION_EXTERNAL_MAPPING " +
            "SET REFERENCE_ARTIFACT = ?, LAST_UPDATED_TIME = ? WHERE UUID = ?";

    public static final String DELETE_SUBSCRIPTION_EXTERNAL_MAPPING_SQL = "DELETE FROM AM_SUBSCRIPTION_EXTERNAL_MAPPING " +
            "WHERE SUBSCRIPTION_UUID = ? AND GATEWAY_ENV_ID = ?";

    public static final String DELETE_SUBSCRIPTION_EXTERNAL_MAPPINGS_SQL = "DELETE FROM AM_SUBSCRIPTION_EXTERNAL_MAPPING " +
            "WHERE SUBSCRIPTION_UUID = ?";

    public static final String GET_SUBSCRIPTION_EXTERNAL_MAPPINGS_SQL = "SELECT UUID, GATEWAY_ENV_ID, " +
            "REFERENCE_ARTIFACT, CREATED_TIME, LAST_UPDATED_TIME " +
            "FROM AM_SUBSCRIPTION_EXTERNAL_MAPPING WHERE SUBSCRIPTION_UUID = ?";

    public static final String CHECK_SUBSCRIPTION_EXTERNAL_MAPPING_EXISTS_SQL = "SELECT 1 FROM " +
            "AM_SUBSCRIPTION_EXTERNAL_MAPPING WHERE SUBSCRIPTION_UUID = ? AND GATEWAY_ENV_ID = ?";

    // External Credential SQL Statements
    public static final String ADD_EXTERNAL_CREDENTIAL_SQL = "INSERT INTO AM_EXTERNAL_CREDENTIAL " +
            "(UUID, MAPPING_UUID, APPLICATION_ID, API_ID, AUTHZ_USER, GATEWAY_ENV_ID, " +
            "NAME, EXTERNAL_SUBSCRIPTION_ID, REFERENCE_ARTIFACT) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

    public static final String GET_EXTERNAL_CREDENTIALS_BY_MAPPING_SQL = "SELECT UUID, MAPPING_UUID, APPLICATION_ID, " +
            "API_ID, AUTHZ_USER, GATEWAY_ENV_ID, NAME, EXTERNAL_SUBSCRIPTION_ID, REFERENCE_ARTIFACT, " +
            "CREATED_TIME, LAST_UPDATED_TIME " +
            "FROM AM_EXTERNAL_CREDENTIAL WHERE MAPPING_UUID = ?";

    public static final String GET_EXTERNAL_CREDENTIALS_BY_SCOPE_SQL = "SELECT UUID, MAPPING_UUID, APPLICATION_ID, " +
            "API_ID, AUTHZ_USER, GATEWAY_ENV_ID, NAME, EXTERNAL_SUBSCRIPTION_ID, REFERENCE_ARTIFACT, " +
            "CREATED_TIME, LAST_UPDATED_TIME FROM AM_EXTERNAL_CREDENTIAL " +
            "WHERE API_ID = ? AND APPLICATION_ID = ? AND GATEWAY_ENV_ID = ? " +
            "ORDER BY COALESCE(LAST_UPDATED_TIME, CREATED_TIME) DESC";

    public static final String GET_EXTERNAL_CREDENTIAL_BY_UUID_SQL = "SELECT UUID, MAPPING_UUID, APPLICATION_ID, " +
            "API_ID, AUTHZ_USER, GATEWAY_ENV_ID, NAME, EXTERNAL_SUBSCRIPTION_ID, REFERENCE_ARTIFACT, " +
            "CREATED_TIME, LAST_UPDATED_TIME " +
            "FROM AM_EXTERNAL_CREDENTIAL WHERE UUID = ?";

    public static final String UPDATE_EXTERNAL_CREDENTIAL_SQL = "UPDATE AM_EXTERNAL_CREDENTIAL " +
            "SET EXTERNAL_SUBSCRIPTION_ID = ?, REFERENCE_ARTIFACT = ?, LAST_UPDATED_TIME = ? WHERE UUID = ?";

    public static final String DELETE_EXTERNAL_CREDENTIAL_SQL = "DELETE FROM AM_EXTERNAL_CREDENTIAL WHERE UUID = ?";

    public static final String GET_SUBSCRIPTION_BY_EXTERNAL_ID_SQL = "SELECT m.SUBSCRIPTION_UUID " +
            "FROM AM_EXTERNAL_CREDENTIAL c " +
            "INNER JOIN AM_SUBSCRIPTION_EXTERNAL_MAPPING m ON c.MAPPING_UUID = m.UUID " +
            "WHERE c.EXTERNAL_SUBSCRIPTION_ID = ? AND m.GATEWAY_ENV_ID = ?";

    public static final String GET_API_CREDENTIAL_SUMMARIES_SQL = "SELECT s.UUID AS SUBSCRIPTION_UUID, " +
            "m.UUID AS MAPPING_UUID, m.REFERENCE_ARTIFACT AS MAPPING_ARTIFACT, " +
            "c.REFERENCE_ARTIFACT AS CREDENTIAL_ARTIFACT, " +
            "c.NAME, c.UUID AS CREDENTIAL_UUID, " +
            "COALESCE(c.LAST_UPDATED_TIME, c.CREATED_TIME) AS LAST_UPDATED_TIME, " +
            "c.APPLICATION_ID, a.NAME AS APP_NAME, " +
            "ap.API_UUID AS API_UUID, ap.API_NAME AS API_NAME, ap.API_DISPLAY_NAME AS API_DISPLAY_NAME, " +
            "ap.API_VERSION AS API_VERSION, ap.STATUS AS API_LIFECYCLE_STATUS, ap.API_TYPE AS API_TYPE " +
            "FROM AM_EXTERNAL_CREDENTIAL c " +
            "INNER JOIN AM_APPLICATION a ON a.UUID = c.APPLICATION_ID " +
            "LEFT JOIN AM_API ap ON ap.API_UUID = c.API_ID " +
            "LEFT JOIN AM_SUBSCRIPTION s ON s.API_ID = ap.API_ID AND s.APPLICATION_ID = a.APPLICATION_ID " +
            "LEFT JOIN AM_SUBSCRIPTION_EXTERNAL_MAPPING m ON c.MAPPING_UUID = m.UUID " +
            "WHERE c.API_ID = ? AND c.GATEWAY_ENV_ID = ? " +
            "ORDER BY COALESCE(c.LAST_UPDATED_TIME, c.CREATED_TIME) DESC";

    public static final String GET_APPLICATION_CREDENTIAL_SUMMARIES_SQL = "SELECT s.UUID AS SUBSCRIPTION_UUID, " +
            "m.UUID AS MAPPING_UUID, m.REFERENCE_ARTIFACT AS MAPPING_ARTIFACT, " +
            "c.REFERENCE_ARTIFACT AS CREDENTIAL_ARTIFACT, " +
            "c.NAME, c.UUID AS CREDENTIAL_UUID, " +
            "COALESCE(c.LAST_UPDATED_TIME, c.CREATED_TIME) AS LAST_UPDATED_TIME, " +
    ...
    public static final String GET_API_SUBSCRIPTION_SUMMARIES_SQL = "SELECT s.UUID AS SUBSCRIPTION_UUID, " +
            "m.UUID AS MAPPING_UUID, m.REFERENCE_ARTIFACT AS MAPPING_ARTIFACT, " +
            "COALESCE(m.CREATED_TIME, s.CREATED_TIME) AS CREATED_TIME, " +
            "COALESCE(m.LAST_UPDATED_TIME, m.CREATED_TIME, s.CREATED_TIME) AS LAST_UPDATED_TIME, " +
            "s.APPLICATION_ID, a.NAME AS APP_NAME, s.SUB_STATUS, " +
            "(SELECT COUNT(*) FROM AM_EXTERNAL_CREDENTIAL c WHERE c.MAPPING_UUID = m.UUID) AS CREDENTIAL_COUNT " +
            "FROM AM_SUBSCRIPTION s " +
            "INNER JOIN AM_APPLICATION a ON s.APPLICATION_ID = a.APPLICATION_ID " +
            "LEFT JOIN AM_SUBSCRIPTION_EXTERNAL_MAPPING m ON m.SUBSCRIPTION_UUID = s.UUID AND m.GATEWAY_ENV_ID = ? " +
            "WHERE s.API_ID = (SELECT API_ID FROM AM_API WHERE API_UUID = ?) " +
            "ORDER BY COALESCE(m.CREATED_TIME, s.CREATED_TIME) DESC";

    // API Federation Config SQL Statements
    public static final String ADD_API_FEDERATION_CONFIG_SQL = "INSERT INTO AM_API_FEDERATION_CONFIG " +
            "(API_UUID, GATEWAY_ENV_ID, PUBLISHER_CURATED_CONFIG, " +
            "GATEWAY_SNAPSHOT_HASH) VALUES (?, ?, ?, ?)";

    public static final String GET_API_FEDERATION_CONFIG_SQL = "SELECT " +
            "PUBLISHER_CURATED_CONFIG, GATEWAY_SNAPSHOT_HASH, " +
            "CREATED_TIME, LAST_UPDATED_TIME, PUBLISHER_REVIEWED_TIME " +
            "FROM AM_API_FEDERATION_CONFIG WHERE API_UUID = ? AND GATEWAY_ENV_ID = ?";

    public static final String UPDATE_API_FEDERATION_CONFIG_HASH_SQL =
            "UPDATE AM_API_FEDERATION_CONFIG SET GATEWAY_SNAPSHOT_HASH = ?, " +
            "LAST_UPDATED_TIME = ? WHERE API_UUID = ? AND GATEWAY_ENV_ID = ?";

    public static final String UPDATE_API_FEDERATION_CONFIG_PUBLISHER_SQL =
            "UPDATE AM_API_FEDERATION_CONFIG SET PUBLISHER_CURATED_CONFIG = ?, " +
            "GATEWAY_SNAPSHOT_HASH = COALESCE(?, GATEWAY_SNAPSHOT_HASH), " +
            "PUBLISHER_REVIEWED_TIME = COALESCE(?, PUBLISHER_REVIEWED_TIME), LAST_UPDATED_TIME = ? " +
            "WHERE API_UUID = ? AND GATEWAY_ENV_ID = ?";

    public static final String CHECK_API_FEDERATION_CONFIG_EXISTS_SQL = "SELECT 1 FROM " +
            "AM_API_FEDERATION_CONFIG WHERE API_UUID = ? AND GATEWAY_ENV_ID = ?";

    public static final String DELETE_API_FEDERATION_CONFIG_SQL = "DELETE FROM AM_API_FEDERATION_CONFIG " +
            "WHERE API_UUID = ? AND GATEWAY_ENV_ID = ?";
    ...
    public static final String GET_GATEWAY_ENV_ID_FROM_EXTERNAL_MAPPING_SQL =
            "SELECT GATEWAY_ENV_ID FROM AM_API_EXTERNAL_API_MAPPING WHERE API_ID = ?";
    ...
    public static final String GET_API_KEY_DETAILS_FROM_KEY_UUID_SQL =
            "SELECT K.NAME, K.API_KEY_HASH, K.KEY_TYPE, K.API_KEY_PROPERTIES, K.AUTHZ_USER, K.VALIDITY_PERIOD, " +
                    "K.LAST_USED, KM.API_UUID, A.ORGANIZATION FROM AM_API_KEY K " +
                    "LEFT JOIN AM_API_KEY_API_MAPPING KM ON K.API_KEY_UUID = KM.API_KEY_UUID " +
                    "LEFT JOIN AM_API A ON KM.API_UUID = A.API_UUID " +
    ...
    public static final String GET_API_UUID_AND_TYPE_FOR_ASSOCIATION_VIA_APP_SQL =
            "SELECT KM.API_UUID, K.NAME, K.KEY_TYPE, K.API_KEY_HASH, K.API_KEY_PROPERTIES, " +
                    "APP.APPLICATION_ID AS APPLICATION_ID " +
                    "FROM AM_API_KEY K " +
                    "JOIN AM_API_KEY_API_MAPPING KM ON K.API_KEY_UUID = KM.API_KEY_UUID " +
                    "JOIN AM_API_KEY_APPLICATION_MAPPING AM ON K.API_KEY_UUID = AM.API_KEY_UUID " +
    ...
    public static final String GET_KEY_DETAILS_FOR_ASSOCIATION_SQL =
            "SELECT K.NAME, K.KEY_TYPE, K.API_KEY_HASH, K.API_KEY_PROPERTIES, " +
                    "COALESCE(APP.APPLICATION_ID, APP_PROVIDED.APPLICATION_ID) AS APPLICATION_ID, " +
